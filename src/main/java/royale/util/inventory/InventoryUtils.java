...
import royale.mixin.ClientWorldAccessor;
...
for (int i = 0; i < 46; i++) {
ItemStack stack = mc.player.getInventory().getStack(i);
EquippableComponent component = (EquippableComponent)stack.get(DataComponentTypes.EQUIPPABLE);
...
Stream<Slot> stream = mc.player.currentScreenHandler.slots.stream().filter(filter);
...
for (i = 36; i <= 44; i++) {
Slot slot = mc.player.playerScreenHandler.getSlot(i);
if (slot != null && !slot.getStack().isEmpty() && slot
...
Slot slot = mc.player.playerScreenHandler.getSlot(i);
if (slot != null && !slot.getStack().isEmpty() && slot
.getStack().getItem() == Items.TOTEM_OF_UNDYING && slot
.getStack().hasEnchantments()) {
return slot;
}
} 
for (i = 9; i <= 35; i++) {
...
else if (enchantedTotem == null) { enchantedTotem = slot; }
}
} 
for (i = 9; i <= 35; i++) {
Slot slot = mc.player.playerScreenHandler.getSlot(i);
...
return;  int syncId = mc.player.playerScreenHandler.syncId;
mc.interactionManager.clickSlot(syncId, slotId, 40, SlotActionType.SWAP, (PlayerEntity)mc.player);
}
...
return;  mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, slot, button, type, (PlayerEntity)mc.player);
}
public static void selectSlot(int slot) {
if (mc.player == null || slot < 0 || slot > 8)
return;  if (mc.player.getInventory().getSelectedSlot() != slot) {
mc.player.getInventory().setSelectedSlot(slot);
}
}
public static void selectSlotSilent(int slot) {
if (mc.player == null || mc.getNetworkHandler() == null || slot < 0 || slot > 8)
return;  mc.getNetworkHandler().sendPacket((Packet)new UpdateSelectedSlotC2SPacket(slot));
}
...
mc.getNetworkHandler().sendPacket((Packet)new UpdateSelectedSlotC2SPacket(hotbarSlot));
}
sendUsePacket(Hand.MAIN_HAND);
if (hotbarSlot != currentSlot) {
mc.getNetworkHandler().sendPacket((Packet)new UpdateSelectedSlotC2SPacket(currentSlot));
}
}
...
mc.getNetworkHandler().sendPacket((Packet)new PlayerInteractItemC2SPacket(hand, sequence, mc.player
.getYaw(), mc.player
.getPitch()));
pendingUpdateManager.close();
} catch (Exception e) {
mc.getNetworkHandler().sendPacket((Packet)new PlayerInteractItemC2SPacket(hand, 0, mc.player
.getYaw(), mc.player
.getPitch()));
} 
}
...
if (mc.player == null || mc.interactionManager == null)
return;  mc.interactionManager.interactItem((PlayerEntity)mc.player, hand);
}
public static void closeScreen() {
if (mc.player == null || mc.getNetworkHandler() == null)
return;  mc.getNetworkHandler().sendPacket((Packet)new CloseHandledScreenC2SPacket(mc.player.currentScreenHandler.syncId));
}
public static boolean isScreenOpen() {
return (mc.currentScreen != null && !(mc.currentScreen instanceof net.minecraft.client.gui.screen.ChatScreen));
}
