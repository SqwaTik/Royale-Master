...
implements IMinecraft {
    ...
    public static Vec3d worldSpaceToScreenSpace(Vec3d pos) {
        ...
        return new Vec3d((double)(target.x / (float)mc.getWindow().getScaleFactor()), (double)(((float)displayHeight - target.y) / (float)mc.getWindow().getScaleFactor()), (double)target.z);
    }
    ...
    private static double getViewZ(Vec3d pos, Vec3d cameraPos) {
    ...
    private static ClipResult worldSpaceToClipSpaceDouble(Vec3d pos, Vec3d cameraPos) {
        double deltaX = pos.x - cameraPos.x;
        double deltaY = pos.y - cameraPos.y;
        double deltaZ = pos.z - cameraPos.z;
        Matrix4d worldSpace = Projection.toMatrix4d(Render3D.lastWorldSpaceMatrix);
    ...
    private static Vec3d clipToScreenDouble(ClipResult clip, int[] viewport, int displayHeight, double scale) {
        double w = clip.w;
        if (Math.abs(w) < 1.0E-14) {
            return new Vec3d((double)viewport[2] / scale / 2.0, (double)displayHeight / scale / 2.0, 0.0);
        }
        double invW = 1.0 / w;
        ...
        return new Vec3d(screenX, screenY, w > 0.0 ? screenZ : -1.0);
    }

    public static double getDistanceToGround() {
        if (Projection.mc.player == null || Projection.mc.world == null) {
            return 256.0;
        }
        for (double y = Projection.mc.player.getY(); y > 0.0; y -= 0.1) {
            if (Projection.mc.world.getBlockState(Projection.mc.player.getBlockPos().down((int)(Projection.mc.player.getY() - y + 1.0))).isAir()) continue;
    ...
    public static Vector4d getVector4D(Entity ent) {
        return Projection.getVector4D(ent, mc.getRenderTickCounter().getTickProgress(true));
    }
    ...
    public static Vector4d getVector4D(Entity ent, float tickDelta) {
        ...
        Vec3d[] corners = new Vec3d[]{new Vec3d(box.minX, box.minY, box.minZ), new Vec3d(box.minX, box.minY, box.maxZ), new Vec3d(box.maxX, box.minY, box.minZ), new Vec3d(box.maxX, box.minY, box.maxZ), new Vec3d(box.minX, box.maxY, box.minZ), new Vec3d(box.minX, box.maxY, box.maxZ), new Vec3d(box.maxX, box.maxY, box.minZ), new Vec3d(box.maxX, box.maxY, box.maxZ)};
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            ++visibleCount;
            Vec3d screen = Projection.clipToScreenDouble(clip, viewport, displayHeight, scale);
        ...
        for (int[] edge : edges) {
            ...
            Vec3d screen = Projection.clipToScreenDouble(clipped, viewport, displayHeight, scale);
            double px = Projection.clampScreenX(screen.x, screenW);
            double py = Projection.clampScreenY(screen.y, screenH);
            minX = Math.min(minX, px);
            minY = Math.min(minY, py);
    ...
    private static boolean isPointInFrontDouble(Vec3d point, Vec3d cameraPos, Camera camera) {
        double lookZ;
        double lookY;
    ...
    public static boolean canSee(Vec3d vec3d) {
        Camera camera = Projection.mc.gameRenderer.getCamera();
        if (camera == null) {
            return false;
        }
        Angle angle = MathAngle.fromVec3d(vec3d.subtract(camera.getCameraPos()));
        return Math.abs(MathHelper.wrapDegrees((float)(angle.getYaw() - camera.getYaw()))) < 90.0f && Math.abs(MathHelper.wrapDegrees((float)(angle.getPitch() - camera.getPitch()))) < 60.0f || Projection.canSee(new Box(BlockPos.ofFloored((Position)vec3d)));
    }
    ...
    public static boolean canSee(Box box) {
        ...
        Vec3d cameraPos = camera.getCameraPos();
        Matrix4f viewMatrix = new Matrix4f().rotation((Quaternionfc)camera.getRotation());
        float fov = Math.max(2.0f, OptionValueUtil.toFloat(Projection.mc.options.getFov().getValue(), 70.0f));
        Matrix4f projectionMatrix = Projection.mc.gameRenderer.getBasicProjectionMatrix(fov);
    ...
    public static boolean isInFrontOfCamera(Vec3d worldPos) {
        Camera camera = Projection.mc.gameRenderer.getCamera();
        if (camera == null || !camera.isReady()) {
            return false;
        }
        return Projection.isPointInFrontDouble(worldPos, camera.getCameraPos(), camera);
    }
