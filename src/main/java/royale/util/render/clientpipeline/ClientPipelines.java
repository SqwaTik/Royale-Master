package royale.util.render.clientpipeline;
...
static {
ROMB_ESP = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(ROMB_ESP_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(1536).build();
...
static {
GHOSTS_ESP = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(GHOSTS_ESP_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(1536).build();
return RenderLayer.of("wtex", setup);
});
}
...
static {
CHAIN_ESP = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(CHAIN_ESP_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(1536).build();
return RenderLayer.of("wtex", setup);
});
}
...
.build());
public static final RenderLayer CRYSTAL_FILLED = RenderLayer.of("crystal_filled", 
...
.withVertexFormat(VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS)
.build());
public static final RenderLayer CRYSTAL_GLOW = RenderLayer.of("crystal_glow", 
...
static {
BLOOM_ESP = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(BLOOM_ESP_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(2048).build();
...
.build());
public static final RenderLayer CHINA_HAT = RenderLayer.of("china_hat", 
...
.build());
public static final RenderLayer CHINA_HAT_OUTLINE = RenderLayer.of("china_hat_outline", 
...
}).withLocation(Identifier.of("royale", "world_particles_color"))
.withVertexFormat(VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS)
.withCull(false)
.withDepthTestFunction(DepthTestFunction.LEQUAL_DEPTH_TEST)
.withDepthWrite(false)
.withBlend(BlendFunction.LIGHTNING)
.build());
public static final RenderLayer WORLD_PARTICLES_QUADS = RenderLayer.of("world_particles_cube", 
...
RenderPipeline.builder(new RenderPipeline.Snippet[] { RenderPipelines.POSITION_COLOR_SNIPPET
}).withLocation(Identifier.of("royale", "world_particles_lines"))
...
.withBlend(BlendFunction.LIGHTNING)
.build());
public static final RenderLayer WORLD_PARTICLES_LINES = RenderLayer.of("world_particles_lines", 
...
}).withLocation(Identifier.of("royale", "world_particles_glow"))
.withVertexFormat(VertexFormats.POSITION_TEXTURE_COLOR, VertexFormat.DrawMode.QUADS)
.withCull(false)
.withDepthTestFunction(DepthTestFunction.LEQUAL_DEPTH_TEST)
.withDepthWrite(false)
...
static {
WORLD_PARTICLES_GLOW = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(WORLD_PARTICLES_GLOW_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(2048).build();
...
static {
GUI_ARROW_BLEND = Util.memoize(texture -> {
RenderSetup setup = RenderSetup.builder(GUI_ARROW_BLEND_PIPELINE).texture("Sampler0", texture).translucent().expectedBufferSize(256).build();
