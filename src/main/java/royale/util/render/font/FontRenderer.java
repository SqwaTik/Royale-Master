...
{
private static final Logger LOGGER = LoggerFactory.getLogger("royale/FontRenderer");
...
Identifier jsonId = Identifier.of("royale", "fonts/" + path + ".json");
Identifier textureId = Identifier.of("royale", "fonts/" + path + ".png");
...
if (this.initialized)
return;  LOGGER.info("Initializing {} fonts...", Integer.valueOf(this.fonts.size()));
long startTime = System.currentTimeMillis();
for (Map.Entry<String, FontAtlas> entry : this.fonts.entrySet()) {
((FontAtlas)entry.getValue()).forceLoad();
}
this.initialized = true;
LOGGER.info("All fonts initialized in {}ms", Long.valueOf(System.currentTimeMillis() - startTime));
}
...
if (atlas == null)
return;  this.pipeline.drawText(atlas, text, x, y, size, color, 0.0F, 0, 0.0F);
}
public void drawText(String fontName, String text, float x, float y, float size, int color, float rotation) {
FontAtlas atlas = this.fonts.get(fontName);
if (atlas == null)
return;  this.pipeline.drawText(atlas, text, x, y, size, color, 0.0F, 0, rotation);
}
...
FontAtlas atlas = this.fonts.get(fontName);
if (atlas == null)
return;  this.pipeline.drawText(atlas, text, x, y, size, color, outlineWidth, outlineColor, 0.0F);
}
...
FontAtlas atlas = this.fonts.get(fontName);
if (atlas == null)
return;  this.pipeline.drawText(atlas, text, x, y, size, color, outlineWidth, outlineColor, rotation);
}
public void drawCenteredText(String fontName, String text, float x, float y, float size, int color) {
FontAtlas atlas = this.fonts.get(fontName);
if (atlas == null)
return;  float width = this.pipeline.getTextWidth(atlas, text, size);
